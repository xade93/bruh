import {
  ...
  describe("Lock", function () {
    ...
    async function prepareDutchAuction() {
      ...
      const lock = await hre.viem.deployContract("DutchAuction", [1000n, 500n, 25n]);
    ...
    describe("Deployment", function () {
      ...
      it("Should set correct initial values", async function () {
        ...
        expect(await lock.read.currentPrice()).to.equal(0n);
      ...
      it("Should correctly decrease price when no transaction is made", async function () {
        const { lock } = await loadFixture(prepareDutchAuction);
        await lock.write.startAuction();
        await hre.network.provider.send("hardhat_mine", ["0x100"]); // mines 256 blocks, equal to 10 decreasement
